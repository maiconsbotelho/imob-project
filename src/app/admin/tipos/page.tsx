...
export default function AdminPropertyTypes() {
  ...
  const toggleType = async (type: PropertyType, checked: boolean) => {
    // Optimistic update
    setTypes((prev) =>
      prev.map((t) => (t.id === type.id ? { ...t, active: checked } : t))
    );
  ...
  const toggleAllTypes = async (checked: boolean) => {
    ...
    try {
      ...
      toast.success(
        checked ? "Todos os tipos ativados" : "Todos os tipos desativados"
      );
    } catch (error) {
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          ...
          <p className="text-gray-600">
            Configure os tipos de imóveis disponíveis no filtro
          </p>
        </div>
      </div>
